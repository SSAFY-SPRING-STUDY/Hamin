...
public class AuthService {
    ...
    public LoginResponse login(LoginRequest request) {
        ...
        MemberEntity member = memberRepository.findByUsername(request.username())
                .orElseThrow(() -> new RuntimeException("올바르지 않은 username"));
        ...
        // 불일치한다면
        throw new RuntimeException("올바르지 않은 Password");
    }
    ...
    public Long getMemberId(String sessionKey) {
        return sessionManager.getMemberId(sessionKey).orElseThrow(
                () -> new RuntimeException("ID 조회 불가")
        );
    }
}